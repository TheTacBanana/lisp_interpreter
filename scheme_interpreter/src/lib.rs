...
impl InterpreterContext {
    ...
    pub fn interpret(&mut self, ast: AST) -> InterpreterResult<()> {
        match ast {
            AST::Operation(op, params) => self.interpret_operation(*op, params),
            AST::Identifier(ident) => {
                self.push_data(self.resolve_identifier(&ident)?);
                Ok(())
            },
            AST::Literal(_) => todo!(),
            AST::List(head, tail) => todo!(),
            AST::EmptyList => todo!(),
        }
    }
    ...
    pub fn interpret_operation(&mut self, op: AST, mut body: Vec<AST>) -> InterpreterResult<()> {
        match op {
            AST::Identifier(i) if &i == "define" => {
                ...
                let (ident, object) = match body.next().unwrap() {
                    ...
                    AST::Identifier(ident) => {
                        ...
                        let ObjectPointer::Object(object) = self.pop_data()? else { return Err(InterpreterError::ExpectedResult)};
                        (ident, object)
                    },
                    // Define a function
                    AST::Operation(op_name, mut op_params) => {
                        let AST::Identifier(op_name) = *op_name else { panic!("Expected Identifier {op_name}") };
                        ...
                        (op_name, Object::Func(Func::Defined(Some(op_name), param_names, body.next().unwrap())))
                    },
            ...
            AST::Identifier(ident) => {
                ...
                let Object::Func(func) = self.deref_pointer(&pointer)? else { return Err(InterpreterError::PointerIsNotFn) };
    ...
    pub fn pop_data(&mut self) -> InterpreterResult<ObjectPointer> {
        self.data_stack.pop().ok_or(InterpreterError::EmptyDataStack)
    }
